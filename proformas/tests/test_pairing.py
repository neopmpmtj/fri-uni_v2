...
from proformas.services import add_line, change_proforma, create_draft, issue_proforma, update_draft
...
def _two_port_system(staff_user, site, indoor):
    power_12k, _ = Power.objects.get_or_create(power=12000, unit="BTU")
    second = Item.objects.create(
        sub_family=indoor.sub_family,
        brand=indoor.brand,
    ...
    proforma = create_draft(site, staff_user, discount_percent=0)
    parent = add_line(proforma, outdoor, staff_user)
    return proforma, parent, indoor, second


@pytest.mark.unit
@pytest.mark.django_db
def test_issue_multi_requires_full_indoor_count(staff_user, site, indoor):
    proforma, parent, indoor, _second = _two_port_system(staff_user, site, indoor)
    add_line(proforma, indoor, staff_user, parent_line=parent)
    with pytest.raises(ValidationError, match="needs 2 to 2 indoor"):
        issue_proforma(proforma, staff_user)


@pytest.mark.unit
@pytest.mark.django_db
def test_override_checks_allows_issue_with_one_indoor(staff_user, site, indoor):
    proforma, parent, indoor, _second = _two_port_system(staff_user, site, indoor)
    add_line(proforma, indoor, staff_user, parent_line=parent)
    update_draft(proforma, staff_user, override_checks=True)
    issued = issue_proforma(proforma, staff_user)
    assert issued.override_checks is True
    assert issued.lines.filter(parent_line__isnull=False).count() == 1


@pytest.mark.unit
@pytest.mark.django_db
def test_override_checks_allows_extra_indoor_on_multi(staff_user, site, indoor):
    proforma, parent, indoor, second = _two_port_system(staff_user, site, indoor)
    add_line(proforma, indoor, staff_user, parent_line=parent)
    add_line(proforma, second, staff_user, parent_line=parent)
    with pytest.raises(ValidationError, match="only has 2 indoor port"):
        add_line(proforma, indoor, staff_user, parent_line=parent)
    update_draft(proforma, staff_user, override_checks=True)
    extra = add_line(proforma, indoor, staff_user, parent_line=parent)
    assert extra.parent_line_id == parent.pk
    issued = issue_proforma(proforma, staff_user)
    assert issued.lines.filter(parent_line__isnull=False).count() == 3
    revised = change_proforma(issued, staff_user)
    assert revised.override_checks is True
    assert revised.lines.filter(parent_line__isnull=False).count() == 3


@pytest.mark.unit
@pytest.mark.django_db
def test_override_checks_does_not_relax_split(staff_user, site, indoor):
    proforma = create_draft(site, staff_user)
    outdoor = indoor.outdoor_matches.get().outdoor
    parent = add_line(proforma, outdoor, staff_user)
    update_draft(proforma, staff_user, override_checks=True)
...
@pytest.mark.integration
@pytest.mark.django_db
def test_draft_page_has_override_checks_checkbox(client, staff_user, site):
    client.force_login(staff_user)
    proforma = create_draft(site, staff_user)
    response = client.get(reverse("proforma_detail", args=[proforma.pk]))
    assert response.status_code == 200
    html = response.content.decode()
    assert "Override checks" in html
    assert 'name="override_checks"' in html
